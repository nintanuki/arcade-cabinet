# Copilot Instructions for Ninja Frog
...
This game is a **standalone project**. It happens to live inside the Arcade Cabinet repo, but its code is agnostic to the launcher: running `python main.py` from this folder must always work on its own. Do not import launcher modules, do not assume the launcher exists, and do not edit files outside this folder from a Ninja Frog change.
...
- `GameManager` ([main.py](../main.py)) must stay thin. Offload responsibilities to dedicated classes (`CollisionManager`, `RenderManager`, `Camera`, `Player`, `Enemy`, `CRT`).
...
- Asset paths live in `AssetPaths` in `settings.py`. Do not hard-code asset filenames in gameplay or rendering code.
- Tile data lives in [tile_maps.py](../tile_maps.py). New solid tile types are added by extending `TILE_LEGEND`, never by hard-coding symbol checks elsewhere.

## File and function layout

- Inside a class, group functions by role (init, input, physics, animation, render).
...
- Do not add `print()` calls to gameplay code. The existing animation-frame-count print in `Player.__init__` is debug scaffolding and should be removed when the project leaves prototype phase.
...
- The player spawns on the floor, can run/walk left and right, and can jump.
- The player collides with all `solid` tile symbols defined in `TILE_LEGEND`.
- The snail enemy walks back and forth; jumping on it bounces the player.
- The camera follows the player and clamps to the map edges.
- `Z` toggles between zoomed-in gameplay view and zoomed-out debug view.
- `F11` and Select toggle fullscreen.
