...
class Laser(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, speed, colors, width, should_grow=False, is_piercing=False):
        ...
        self.image = pygame.Surface((LaserSettings.LENGTH, self.current_width))
    ...
    def move(self, speed_multiplier=1.0):
        """Moves the laser horizontally based on its speed. Called every frame in update()."""
        self.pos_x += self.speed * speed_multiplier
    ...
    def animate_rainbow(self):
        ...
        for segment_index in range(LaserSettings.RAINBOW_SEGMENTS):
            # Offset the hue for each segment based on its position along the beam.
            seg_hue = (self.hue + (segment_index * LaserSettings.RAINBOW_SEGMENT_SHIFT)) % 360

            color = pygame.Color(0)
            color.hsva = (seg_hue, 100, 100, 100)

            # Fill a vertical slice of the beam surface with this segment's hue.
            segment_rect = pygame.Rect(
                segment_index * LaserSettings.SEGMENT_SIZE,
                0,
                LaserSettings.SEGMENT_SIZE,
                self.current_width,
            )
    ...
    def destroy_if_offscreen(self):
        """Destroys the laser if it goes off the left or right edge of the screen. Called every frame in update()."""
        if self.rect.right < 0 or self.rect.left > ScreenSettings.WIDTH:
            self.kill()
...
class BombProjectile(pygame.sprite.Sprite):
    ...
    def move(self, speed_multiplier=1.0):
        """Advance projectile rightward with frame-scaled world speed.
    ...
    def destroy_if_offscreen(self):
        """Remove projectile once it leaves the right edge of the screen."""
        if self.rect.left > ScreenSettings.WIDTH:
            self.kill()
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self,pos,audio):
        ...
        # Rotate 90° clockwise so the ship faces right for horizontal play.
        self.original_image = pygame.transform.rotozoom(self.original_image, -90, PlayerSettings.SCALE)
    ...
    def launch_bomb(self):
        ...
        self.bomb_projectiles.add(BombProjectile(self.rect.midright))
        self.bombs -= 1
        return True
    ...
    def shoot_laser(self):
        ...
        if self.laser_level >= 2:
            # Top laser
            self.lasers.add(Laser((self.rect.centerx, self.rect.centery - offset),
            ...
            # Level 1: Single laser from the ship's right edge (nose)
            self.lasers.add(Laser(self.rect.midright, laser_speed,
...
class Alien(pygame.sprite.Sprite):
    ...
    def __init__(self,color,screen_width,screen_height):
        ...
        self.frame_index = 0
        # Rotate all frames 90° clockwise so aliens face left toward the player.
        self.frames = [pygame.transform.rotate(f, -90) for f in self.frames]
        self.image = self.frames[self.frame_index]

        # Position setup: random y position, spawn beyond the right edge.
        y_pos = random.randint(20, self.screen_height - 20)
        x_pos = self.screen_width + random.randint(*AlienSettings.SPAWN_OFFSET)
        self.rect = self.image.get_rect(center=(x_pos, y_pos))
        self.position = pygame.math.Vector2(self.rect.topleft)

        # Movement logic
        # Yellow aliens zigzag vertically
        self.yellow_zigzag_direction = random.choice([-1, 1])  # 1 for down, -1 for up
        self.yellow_zigzag_counter = 0
        # Blue aliens zigzag vertically
        self.blue_zigzag_direction = random.choice([-1, 1])  # 1 for down, -1 for up
        ...
        self.confusion_growth = 0  # Starts at 0, grows to ScreenSettings.WIDTH
    ...
    def calculate_movement(self, speed_multiplier=1.0):
        ...
        if self.can_confuse and not self.has_projected:
            if self.rect.centerx <= AlienSettings.CONFUSION_STOP_X:
        ...
        if self.color == 'red':
            self.apply_movement(-AlienSettings.SPEED['red'] * speed_multiplier, 0)
        elif self.color == 'green':
            self.apply_movement(-AlienSettings.SPEED['green'] * speed_multiplier, 0)
        elif self.color == 'yellow':
            # Zigzag vertically (up/down) while advancing left.
            self.apply_movement(-AlienSettings.SPEED['yellow'] * speed_multiplier,
                                self.yellow_zigzag_direction * 2 * speed_multiplier)
            ...
            self.apply_movement(-AlienSettings.SPEED['blue'] * speed_multiplier,
                                self.blue_zigzag_direction * 2 * speed_multiplier)
            if self.rect.top < 0 or self.rect.bottom > self.screen_height:
    ...
    def update_confusion_beam(self):
        ...
        # Grow the beam toward the left edge of the screen, capped to screen
        # width so it cannot extend forever.
        if self.confusion_growth < ScreenSettings.WIDTH:
            self.confusion_growth += AlienSettings.CONFUSION_BEAM_GROWTH_PER_FRAME

        # Trapezoid: narrow at the alien's left side, fans leftward by a fraction
        # of the current beam length so longer beams look more menacing.
        ...
        beam_polygon = [
            (self.rect.left, self.rect.centery - top_width // 2),
            (self.rect.left, self.rect.centery + top_width // 2),
            (self.rect.left - self.confusion_growth, self.rect.centery + int(bottom_width // 2)),
            (self.rect.left - self.confusion_growth, self.rect.centery - int(bottom_width // 2)),
        ]
    ...
    def destroy(self):
        """Destroys the alien once it passes the configured left-edge margin."""
        if self.rect.right < -AlienSettings.OFFSCREEN_MARGIN:
            self.kill()
...
class PowerUp(pygame.sprite.Sprite):
    ...
    def move(self):
        """Moves the powerup leftward based on its speed. Called every frame in update()."""
        self.rect.x -= PowerupSettings.SPEED
    ...
    def destroy(self):
        """Destroys the powerup if it moves off the left edge of the screen. Called every frame in update()."""
