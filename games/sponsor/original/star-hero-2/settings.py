...
class ScreenSettings:
    ...
    PLAYER_START = (WIDTH // 6, HEIGHT // 2)  # Left-side start for horizontal play
    FPS = 120
...
class AlienSettings:
    ...
    SPAWN_OFFSET = (100, 300)  # pixels beyond the right edge at which aliens spawn
    ...
    CONFUSION_STOP_X = ScreenSettings.WIDTH // 2  # Halfway across screen; blue alien stops here to project confusion beam
...
class LaserSettings:
    ...
    DEFAULT_WIDTH = 4
    LENGTH = 20  # length of a laser along its travel direction (pixels)
    PLAYER_LASER_SPEED = 8   # positive = rightward
    ALIEN_LASER_SPEED = -4   # negative = leftward (toward player)
    COLORS = {
        'single': ('green', 'white'),
        'twin': ('green', 'white'),
        'burst': ('cyan', 'white'),
    ...
    RAINBOW_BEAM_WIDTH = ScreenSettings.HEIGHT  # beam spans full screen height (perpendicular to travel)
    RAINBOW_BEAM_GROWTH_SPEED = 5  # pixels added per frame
    RAINBOW_HUE_STEP = 4
    RAINBOW_SEGMENTS = 5
    # Divide the laser into 5 segments along its length for a color-flow effect
    SEGMENT_SIZE = LENGTH // RAINBOW_SEGMENTS
...
class BombSettings:
    ...
    PROJECTILE_SPEED = 2  # positive = rightward (toward enemies)
    FLASH_SPEED = 220
