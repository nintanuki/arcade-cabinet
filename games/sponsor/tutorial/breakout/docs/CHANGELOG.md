# Change Log

Append-only record of every code change made to Breakout. Read it before making changes so you know the current state of the codebase.

## Format

    ## YYYY-MM-DD HH:MM — short summary

    **File:** path/to/file.py
    **Lines (at time of edit):** 38-52 (modified)
    **Before:**
        [old code]
    **After:**
        [new code]
    **Why:** explanation
    **Editor:** name (AI model used, if any)

## Conventions

* Line numbers reflect the file at the moment of the edit; never go back to "fix" old line numbers.
* Append-only. Never delete history.
* For new files write `(new file)` instead of a line range.
* For deletes write `(deleted)` and put the removed code in "Before" with no "After".

---
