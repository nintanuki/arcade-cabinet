# Copilot Instructions for Pazaak
...
This game is a **standalone project**. It happens to live inside the Arcade Cabinet repo, but its code is agnostic to the launcher: running `python main.py` from this folder must always work on its own. Do not import launcher modules, do not assume the launcher exists, and do not edit files outside this folder from a Pazaak change.

The current state of the codebase is a **scaffold**. Most architecture decisions are still open. When in doubt, mirror the patterns used in `games/sponsor/tribute/game-of-the-amazons/` (the most polished tribute game in this repo) rather than inventing new structures.
...
- `GameManager` ([main.py](../main.py)) must stay thin. As gameplay is added, push responsibilities into dedicated classes (deck, hand, side-deck, AI opponent, audio, view).
...
- Card values, deck sizes, and hand-total thresholds (e.g. `MAX_HAND_TOTAL = 20`) all live in `settings.py`.

## File and function layout

- Inside a class, group functions by role (boot, lifecycle, input, gameplay actions, audio, render, run loop). The scaffold already establishes these section banners — keep them.
...
- Player-facing text in the HUD, table, and overlays should be **ALL CAPS** to match the retro pixel aesthetic. Documentation files stay in normal sentence case.
...
- Window opens at the configured resolution.
- The CRT overlay is visible.
- The controller quit combo and `Esc` exit cleanly.
