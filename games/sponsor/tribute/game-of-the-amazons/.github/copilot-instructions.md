# Copilot Instructions for Game of the Amazons
...
This game is a **standalone project**. It happens to live inside the Arcade Cabinet repo, but its code is agnostic to the launcher: running `python main.py` from this folder must always work on its own. Do not import launcher modules, do not assume the launcher exists, and do not edit files outside this folder from a Game of the Amazons change.
...
- `GameManager` ([main.py](../main.py)) must stay thin. Offload responsibilities to dedicated classes (`Board`, `TurnManager`, `BoardView`, `HUD`, `PieceAnimator`, `ArrowAnimator`, `AudioManager`, `CRT`).
...
- The screen layout is fully derived from `GridSettings.TILE_SIZE` and `UISettings.*` — do not hard-code pixel positions.

## File and function layout

- Inside a class, group functions by role (boot, input, phase transitions, per-frame update, render, main loop).
...
- Player-facing text in the HUD and overlays should be **ALL CAPS** to match the retro pixel aesthetic. Documentation files stay in normal sentence case.
...
- Board renders 10 × 10 with four white queens (bottom row positions) and four black queens (top row positions).
- Cursor moves with arrow keys / D-pad and stays clamped to `[0, 9]` on both axes.
- A WHITE queen can be selected, moved along a valid path, and a valid arrow can be fired afterward.
- The AI (BLACK) takes its turn after WHITE finishes, animating its queen slide and then its arrow.
- Win is detected when a player has no legal move at the start of their turn.
- `Enter` (or Start) restarts a finished match without crashing.
